...
type Args struct {
	Entity string         `json:"entity" jsonschema:"coffee | coffee_domain | donate_rest_controller | portfolio_controller | repository_detail_domain | repository_issue_domain | version"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.GithubProjectIssuesSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "github-project-issues_list",
		Description: "List records from GithubProjectIssues. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "github-project-issues_load",
		Description: "Load a single record from GithubProjectIssues. " +
...
func runOp(client *sdk.GithubProjectIssuesSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.GithubProjectIssuesSDK, name string) (sdk.GithubProjectIssuesEntity, error) {
	...
	case "coffee_domain":
		return client.CoffeeDomain(nil), nil
	case "donate_rest_controller":
		return client.DonateRestController(nil), nil
	case "portfolio_controller":
		return client.PortfolioController(nil), nil
	case "repository_detail_domain":
		return client.RepositoryDetailDomain(nil), nil
	case "repository_issue_domain":
		return client.RepositoryIssueDomain(nil), nil
	case "version":
