...
class CoffeeDomainDirectTest < Minitest::Test
  def test_direct_list_coffee_domain
    setup = coffee_domain_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-coffee_domain", setup[:live] ? "live" : "unit")
    if _should_skip
...
def coffee_domain_direct_setup(mockres)
