...
class PortfolioControllerDirectTest < Minitest::Test
  def test_direct_list_portfolio_controller
    setup = portfolio_controller_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-portfolio_controller", setup[:live] ? "live" : "unit")
    if _should_skip
...
def portfolio_controller_direct_setup(mockres)
