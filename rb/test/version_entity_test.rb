...
class VersionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITHUBPROJECTISSUES_TEST_VERSION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    version_ref01_data_dt0_loaded, err = version_ref01_ent.load(version_ref01_match_dt0, nil)
    assert_nil err
...
def version_basic_setup(extra)
  ...
  entid_env_raw = ENV["GITHUBPROJECTISSUES_TEST_VERSION_ENTID"]
