...
class CoffeeDirectTest < Minitest::Test
  def test_direct_list_coffee
    setup = coffee_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-coffee", setup[:live] ? "live" : "unit")
    if _should_skip
...
def coffee_direct_setup(mockres)
