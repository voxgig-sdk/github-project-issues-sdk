...
class DonateRestControllerEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.DonateRestController(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.DonateRestController(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = donate_rest_controller_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "donate_rest_controller." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITHUBPROJECTISSUES_TEST_DONATE_REST_CONTROLLER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    donate_rest_controller_ref01_data = nil
    if donate_rest_controller_ref01_data_raw.length > 0
      donate_rest_controller_ref01_data = Helpers.to_map(donate_rest_controller_ref01_data_raw[0][1])
    end

    # LIST
    donate_rest_controller_ref01_ent = client.DonateRestController(nil)
    donate_rest_controller_ref01_match = {}

    donate_rest_controller_ref01_list_result = donate_rest_controller_ref01_ent.list(donate_rest_controller_ref01_match, nil)
    assert donate_rest_controller_ref01_list_result.is_a?(Array)

  end
end

def donate_rest_controller_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "donate_rest_controller", "DonateRestControllerTestData.json")
  ...
  entid_env_raw = ENV["GITHUBPROJECTISSUES_TEST_DONATE_REST_CONTROLLER_ENTID"]
