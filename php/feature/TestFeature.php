...
// GithubProjectIssues SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class GithubProjectIssuesTestFeature extends GithubProjectIssuesBaseFeature
{
    ...
    public function init(GithubProjectIssuesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (GithubProjectIssuesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = GithubProjectIssuesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (GithubProjectIssuesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(GithubProjectIssuesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = GithubProjectIssuesParam::call($ctx, $k);
