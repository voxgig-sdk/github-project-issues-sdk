...
class FtCtrl extends GithubProjectIssuesControl
{
...
class FtClient
{
    ...
    private GithubProjectIssuesUtility $utility;

    public function __construct(GithubProjectIssuesUtility $utility, array $options)
    {
    ...
    public function get_utility(): GithubProjectIssuesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GithubProjectIssuesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GithubProjectIssuesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GithubProjectIssuesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GithubProjectIssuesConfig::make_feature($name);
    ...
    private static function buildUrl(GithubProjectIssuesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GithubProjectIssuesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GithubProjectIssuesResponse($fetched);
        }
    ...
    private function fail(GithubProjectIssuesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GithubProjectIssuesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
