// Command GithubProjectIssues-cli is a CLI + REPL for the GithubProjectIssues SDK,
...
const prompt = "github-project-issues"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "coffee coffee_domain donate_rest_controller portfolio_controller repository_detail_domain repository_issue_domain version"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GITHUB_PROJECT_ISSUES_APIKEY (api key), GITHUB_PROJECT_ISSUES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GITHUB_PROJECT_ISSUES_APIKEY carries the API key and
	// GITHUB_PROJECT_ISSUES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GITHUB_PROJECT_ISSUES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GITHUB_PROJECT_ISSUES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGithubProjectIssuesSDK(opts)
