...
func TestRepositoryDetailDomainDirect(t *testing.T) {
	t.Run("direct-list-repository_detail_domain", func(t *testing.T) {
		setup := repository_detail_domainDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-repository_detail_domain", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-repository_detail_domain", func(t *testing.T) {
		setup := repository_detail_domainDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-repository_detail_domain", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type repository_detail_domainDirectSetupResult struct {
...
func repository_detail_domainDirectSetup(mockres any) *repository_detail_domainDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUBPROJECTISSUES_TEST_REPOSITORY_DETAIL_DOMAIN_ENTID"]; ok {
		...
		return &repository_detail_domainDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &repository_detail_domainDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
