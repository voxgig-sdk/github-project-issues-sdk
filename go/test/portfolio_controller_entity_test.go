...
func TestPortfolioControllerEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITHUBPROJECTISSUES_TEST_PORTFOLIO_CONTROLLER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func portfolio_controllerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GITHUBPROJECTISSUES_TEST_PORTFOLIO_CONTROLLER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUBPROJECTISSUES_TEST_PORTFOLIO_CONTROLLER_ENTID"])
