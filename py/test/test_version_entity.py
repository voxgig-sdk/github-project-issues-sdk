    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUBPROJECTISSUES_TEST_VERSION_ENTID JSON to run live")
        client = setup["client"]
        ...
        version_ref01_data_dt0_loaded, err = version_ref01_ent.load(version_ref01_match_dt0, None)
        assert err is None
...
def _version_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUBPROJECTISSUES_TEST_VERSION_ENTID"))
