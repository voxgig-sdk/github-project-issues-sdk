...
class TestDonateRestControllerDirect:

    def test_should_direct_list_donate_rest_controller(self):
        setup = _donate_rest_controller_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-donate_rest_controller", "live" if setup["live"] else "unit")
        if _skip:
...
def _donate_rest_controller_direct_setup(mockres):
