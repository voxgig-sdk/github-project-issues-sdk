...
class TestRepositoryIssueDomainEntity:

    def test_should_create_instance(self):
        testsdk = GithubProjectIssuesSDK.test(None, None)
        ent = testsdk.RepositoryIssueDomain(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.RepositoryIssueDomain(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.RepositoryIssueDomain(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _repository_issue_domain_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "repository_issue_domain." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUBPROJECTISSUES_TEST_REPOSITORY_ISSUE_DOMAIN_ENTID JSON to run live")
        client = setup["client"]
        ...
        repository_issue_domain_ref01_data = None
        if len(repository_issue_domain_ref01_data_raw) > 0:
            repository_issue_domain_ref01_data = helpers.to_map(repository_issue_domain_ref01_data_raw[0][1])

        # LIST
        repository_issue_domain_ref01_ent = client.RepositoryIssueDomain(None)
        repository_issue_domain_ref01_match = {
            "repository": setup["idmap"]["repository01"],
            "username": setup["idmap"]["username01"],
        }

        repository_issue_domain_ref01_list_result = repository_issue_domain_ref01_ent.list(repository_issue_domain_ref01_match, None)
        assert isinstance(repository_issue_domain_ref01_list_result, list)



def _repository_issue_domain_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/repository_issue_domain/RepositoryIssueDomainTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUBPROJECTISSUES_TEST_REPOSITORY_ISSUE_DOMAIN_ENTID"))
