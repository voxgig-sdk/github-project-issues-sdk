...
class TestCoffeeDirect:

    def test_should_direct_list_coffee(self):
        setup = _coffee_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-coffee", "live" if setup["live"] else "unit")
        if _skip:
...
def _coffee_direct_setup(mockres):
