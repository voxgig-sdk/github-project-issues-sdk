...
describe('RepositoryIssueDomainEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'repository_issue_domain.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set GITHUB_PROJECT_ISSUES_TEST_REPOSITORY_ISSUE_DOMAIN_ENTID JSON to run live')
      return
    }
    ...
    const repository_issue_domain_ref01_list = await repository_issue_domain_ref01_ent.list(repository_issue_domain_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['repository_issue_domain01','repository_issue_domain02','repository_issue_domain03','get_repo_issue01','get_repo_issue02','get_repo_issue03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['GITHUB_PROJECT_ISSUES_TEST_REPOSITORY_ISSUE_DOMAIN_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'GITHUB_PROJECT_ISSUES_TEST_REPOSITORY_ISSUE_DOMAIN_ENTID': idmap,
    'GITHUB_PROJECT_ISSUES_TEST_LIVE': 'FALSE',
    'GITHUB_PROJECT_ISSUES_TEST_EXPLAIN': 'FALSE',
    'GITHUB_PROJECT_ISSUES_APIKEY': 'NONE',
  })

  idmap = env['GITHUB_PROJECT_ISSUES_TEST_REPOSITORY_ISSUE_DOMAIN_ENTID']
