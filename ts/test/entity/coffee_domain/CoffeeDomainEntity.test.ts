...
describe('CoffeeDomainEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'coffee_domain.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set GITHUB_PROJECT_ISSUES_TEST_COFFEE_DOMAIN_ENTID JSON to run live')
      return
    }
    ...
    const coffee_domain_ref01_list = await coffee_domain_ref01_ent.list(coffee_domain_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITHUB_PROJECT_ISSUES_TEST_COFFEE_DOMAIN_ENTID']
