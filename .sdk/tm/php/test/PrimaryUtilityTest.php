...
// GithubProjectIssues SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        GithubProjectIssuesSDK $client,
        GithubProjectIssuesUtility $utility
    ): GithubProjectIssuesContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new GithubProjectIssuesContext($ctxmap, null);
    ...
    private static function fixctx(GithubProjectIssuesContext $ctx, GithubProjectIssuesSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?GithubProjectIssuesError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GithubProjectIssuesError($code, $msg);
    }
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new GithubProjectIssuesBaseFeature();
    ...
    {
        $hook_client = GithubProjectIssuesSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends GithubProjectIssuesBaseFeature {
            public $hook_fn = null;
            public function TestHook(GithubProjectIssuesContext $ctx): void
            {
    ...
    {
        $init_client = GithubProjectIssuesSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends GithubProjectIssuesBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new GithubProjectIssuesSDK([
            'system' => [
    ...
    {
        $blocked_client = new GithubProjectIssuesSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new GithubProjectIssuesSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = GithubProjectIssuesSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new GithubProjectIssuesOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = GithubProjectIssuesSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof GithubProjectIssuesError) ? $result->err->msg : (string)$result->err;
