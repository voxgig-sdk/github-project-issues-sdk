...
// GithubProjectIssues SDK retry feature
...
class GithubProjectIssuesRetryFeature extends GithubProjectIssuesBaseFeature
{
    ...
    public function init(GithubProjectIssuesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GithubProjectIssuesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GithubProjectIssuesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GithubProjectIssuesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
