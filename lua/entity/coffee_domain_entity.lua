...
local CoffeeDomainEntity = {}
CoffeeDomainEntity.__index = CoffeeDomainEntity


function CoffeeDomainEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CoffeeDomainEntity)
  self._name = "coffee_domain"
  self._client = client
...
function CoffeeDomainEntity:get_name()
  return self._name
end


function CoffeeDomainEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CoffeeDomainEntity.new(self._client, opts)
end


function CoffeeDomainEntity:data_set(args)
  if args ~= nil then
...
function CoffeeDomainEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CoffeeDomainEntity:match_set(args)
  if args ~= nil then
...
function CoffeeDomainEntity:match_get()
...
function CoffeeDomainEntity:stream(action, args, callopts)
...
function CoffeeDomainEntity:_run_op(ctx, post_done)
  local utility = self._utility
