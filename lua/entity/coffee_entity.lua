...
local CoffeeEntity = {}
CoffeeEntity.__index = CoffeeEntity


function CoffeeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CoffeeEntity)
  self._name = "coffee"
  self._client = client
...
function CoffeeEntity:get_name()
  return self._name
end


function CoffeeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CoffeeEntity.new(self._client, opts)
end


function CoffeeEntity:data_set(args)
  if args ~= nil then
...
function CoffeeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CoffeeEntity:match_set(args)
  if args ~= nil then
...
function CoffeeEntity:match_get()
...
function CoffeeEntity:stream(action, args, callopts)
...
function CoffeeEntity:_run_op(ctx, post_done)
  local utility = self._utility
