...
The Lua SDK for the GithubProjectIssues API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Coffee(nil):list(nil, nil)
...
local result, err = client:GithubProjectIssues(nil):load(
  { id = "test01" }, nil
)
