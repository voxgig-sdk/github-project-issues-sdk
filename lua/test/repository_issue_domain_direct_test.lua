...
describe("RepositoryIssueDomainDirect", function()
  it("should direct-list-repository_issue_domain", function()
    local setup = repository_issue_domain_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-repository_issue_domain", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"repository01", "username01"}) do
